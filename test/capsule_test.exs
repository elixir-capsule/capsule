  ...
  describe "storage!/1" do
    test "returns storage module" do
      assert Capsule.Storages.Mock =
               Capsule.storage!(%Encapsulation{storage: "Elixir.Capsule.Storages.Mock"})
    end

    test "handles storage without Elixir prefix" do
      assert Capsule.Storages.Mock =
               Capsule.storage!(%Encapsulation{storage: "Capsule.Storages.Mock"})
    end

    test "raises error on invalid storage" do
      assert_raise Capsule.Errors.InvalidStorage, fn ->
        Capsule.storage!(%Encapsulation{storage: "what"})
      end
    end
  end
end
